...
class TransformerEncoderLayer(nn.Module):
    def __init__(self, args):
        ...
        self.fc1 = generate_linear(self.embed_dim, args.encoder_ffn_embed_dim)  # 128 x 512
    ...
    def forward(self, state, encoder_padding_mask):
        ...
        '''
        # Oda: state is either the initial embedding or the output of the last transformer block.
...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self,
        ...
        self.head_embed_size = embed_dim // num_attn_heads  # this is d_k in the paper, 64
    ...
    def forward(self,
        ...
        num_time_steps, batch_size, embed_dim = query.size()
        ...
        # attn = torch.zeros(size = (num_time_steps, batch_size, embed_dim))
        attn_weights = torch.zeros(size = (
            self.num_heads, batch_size, num_time_steps, key.size(0))) if need_weights else None
...
class PositionalEmbedding(nn.Module):
    ...
    def forward(self, inputs, incremental_state = None, timestep = None):
        ...
        if incremental_state is not None:
            ...
            return self.weights.index_select(index = self.padding_idx + pos, dim = 0).unsqueeze(1).repeat(batch_size, 1,
                                                                                                          1)
